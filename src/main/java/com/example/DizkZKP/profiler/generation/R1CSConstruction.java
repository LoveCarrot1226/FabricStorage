...
package com.example.DizkZKP.profiler.generation;

import com.example.DizkZKP.algebra.fields.AbstractFieldElementExpanded;
import com.example.DizkZKP.common.Utils;
import com.example.DizkZKP.configuration.Configuration;
import org.apache.spark.api.java.JavaPairRDD;
import com.example.DizkZKP.relations.objects.*;
import com.example.DizkZKP.relations.r1cs.R1CSRelation;
import com.example.DizkZKP.relations.r1cs.R1CSRelationRDD;
import scala.Tuple2;
import scala.Tuple3;
...
public class R1CSConstruction implements Serializable {
    ...
    parallelConstruct(
        ...
        JavaPairRDD<Long, LinearTerm<FieldT>> linearCombinationA = config.sparkContext()
                .parallelize(partitions, numPartitions).flatMapToPair(part -> {
                    final long partSize = part == numPartitions ?
                            totalSize % (totalSize / numPartitions) : totalSize / numPartitions;
        ...
        linearCombinationA = config.sparkContext().parallelize(variablePartitions, numPartitions)
                .flatMapToPair(part -> {
                    final long partSize = part == numPartitions ?
        ...
        linearCombinationB = config.sparkContext().parallelize(variablePartitions, numPartitions)
                .flatMapToPair(part -> {
                    final long partSize = part == numPartitions ?
                            totalVariableSize % (totalVariableSize / numPartitions)
                            : totalVariableSize / numPartitions;
                ...
                .persist(config.storageLevel());
        linearCombinationC = linearCombinationC.union(config.sparkContext().parallelizePairs(
                Collections.singletonList(
                        new Tuple2<>(numConstraints - 1, new LinearTerm<>(numVariables - 1, one)))))
                .persist(config.storageLevel());
        ...
        JavaPairRDD<Long, FieldT> oneFullAssignment = config.sparkContext()
                .parallelize(assignmentPartitions, numExecutors).flatMapToPair(part -> {
                    ...
                    final boolean lastPartition = (totalSize % 2 != 0 && part == numExecutors)
    ...
    /** Linear com.example.DizkZKP.algebra applications */
        ...
        if (i < n2) {
            ...
            switch (t) {
                ...
                case constraintB:
                    ...
                    break;
                case constraintC:
                    constraintArray.add(new Tuple2<>(index, new LinearTerm<>(zOffset + (row * n3 + col) * (n2) + i, one)));
                    break;
                default:
            ...
            if (i == (n2 - 1) || (i == 0 && n2 == 2)) {
                ...
                switch (t) {
                    case constraintA:
                        if (n2 == 2) {
                            constraintArray.add(new Tuple2<>(index, new LinearTerm<>(zOffset + (row * n3 + col) * n2, one)));
                            constraintArray.add(new Tuple2<>(index, new LinearTerm<>(zOffset + (row * n3 + col) * n2 + 1, one)));
                        } else {
                            constraintArray.add(new Tuple2<>(index, new LinearTerm<>(zOffset + (row * n3 + col) * n2 + i + 1, one)));
                            constraintArray.add(new Tuple2<>(index, new LinearTerm<>(sOffset + (row * n3 + col) * (n2 - 1) + i - 1, one)));
                        }
                        break;
                    case constraintB:
                        ...
                        break;
                    case constraintC:
                        constraintArray.add(new Tuple2<>(index, new LinearTerm<>(cOffset + row * n3 + col, one)));
                        break;
                    default:
                        System.out.println("[R1CSConstruction.constraintAssignment] unknown constraint");
                        break;
                ...
                switch (t) {
                    ...
                    case constraintB:
                        ...
                        break;
                    case constraintC:
                        constraintArray.add(new Tuple2<>(index, new LinearTerm<>(sOffset + (row * n3 + col) * (n2 - 1), one)));
                        break;
                    default:
                        System.out.println("[R1CSConstruction.constraintAssignment] unknown constraint");
                        break;
                ...
                switch (t) {
                    ...
                    case constraintB:
                        ...
                        break;
                    case constraintC:
                        constraintArray.add(new Tuple2<>(index, new LinearTerm<>(sOffset + (row * n3 + col) * (n2 - 1) + i, one)));
                        break;
                    default:
                        System.out.println("[R1CSConstruction.constraintAssignment] unknown constraint");
                        break;
    ...
    matmulConstruct(final int n1,
        ...
        JavaPairRDD<Long, LinearTerm<FieldT>> ALC = config.sparkContext()
                .parallelize(partitions, numPartitions).flatMapToPair(part -> {
                    final long partSize = (part == numPartitions && totalSize % 2 != 0) ?
                            totalSize % (totalSize / numPartitions) : totalSize / numPartitions;

                    final ArrayList<Tuple2<Long, LinearTerm<FieldT>>> A = new ArrayList<>();
                    for (long i = 0; i < partSize; i++) {
                        final long index = part * partSize + i;
                        constraintAssignment(A, fieldFactory, index, n1, n2, n3, ConstraintType.constraintA);
                    }
                    return A.iterator();
                });


        JavaPairRDD<Long, LinearTerm<FieldT>> BLC = config.sparkContext()
                .parallelize(partitions, numPartitions).flatMapToPair(part -> {
                    final long partSize = (part == numPartitions && totalSize % 2 != 0) ?
                            totalSize % (totalSize / numPartitions) : totalSize / numPartitions;
                    ...
                    for (long i = 0; i < partSize; i++) {
                        final long index = part * partSize + i;
                        constraintAssignment(B, fieldFactory, index, n1, n2, n3, ConstraintType.constraintB);
                    }
                    return B.iterator();
                });

        JavaPairRDD<Long, LinearTerm<FieldT>> CLC = config.sparkContext()
                .parallelize(partitions, numPartitions).flatMapToPair(part -> {
                    final long partSize = (part == numPartitions && totalSize % 2 != 0) ?
                            totalSize % (totalSize / numPartitions) : totalSize / numPartitions;
                    ...
                    for (long i = 0; i < partSize; i++) {
                        final long index = part * partSize + i;
                        constraintAssignment(C, fieldFactory, index, n1, n2, n3, ConstraintType.constraintC);
                    }
                    return C.iterator();
                });

        // We're going to construct the assignment serially for now. Probably need to implement block-based matrix multiplication.
        ...
        for (long i = startIndex; i < startIndex + cSize; i++) {
            ...
            for (long j = 0; j < n2; j++) {
                // A's row * B's column
                result = result.add(assignment.get((int) (1 + row * n2 + j))._2().mul(assignment.get((int) (1 + n1 * n2 + j * n3 + col))._2()));
            }
        ...
        for (long i = startIndex; i < startIndex + zSize; i++) {
            ...
            FieldT result = assignment.get((int) (1 + row * n2 + idx))._2().mul(assignment.get((int) (1 + n1 * n2 + idx * n3 + col))._2());
        ...
        for (long i = startIndex; i < startIndex + sSize; i++) {
            ...
            if (sIndex == 0) {
                ...
                FieldT result = assignment.get((int) (zIndex))._2().add(assignment.get((int) (zIndex + 1))._2());
                zIndex += 2;
                assignment.add(new Tuple2<>(i, result));
            } else {
        ...
        JavaPairRDD<Long, FieldT> oneFullAssignment = JavaPairRDD.fromJavaRDD(config.sparkContext().parallelize(assignment, numPartitions));
        final R1CSConstraintsRDD<FieldT> constraints = new R1CSConstraintsRDD<>(ALC, BLC, CLC, numConstraints);
        final R1CSRelationRDD<FieldT> r1cs = new R1CSRelationRDD<>(constraints,
                numInputs,
                numAuxiliary);
    ...
    // Z = the products
    // S = the sums
    // Let's assume that we have P^2 partitions. We divide C into P by P blocks, where each block is (n1 / p) x (n3 / p).
    ...
    // Each machine stores (n1 / p) x (n2 / p) + (n2 / p) x (n3 / p) and shuffles (n1 / p) x (n2 / p) * p + (n2 / p) x (n3 / p) * p.
    // Total traffic has p blowup, which is sqrt(M), where M = # of machines.
    //
    // Perhaps not very applicable to our setting but: note that within each machine, if the memory is limited, then outer-product is the right way to calculate the C block matrix
    //
    // The functions here are:
    // - matmulParAssignHelper() actually takes in the shuffled blocks and does the matrix multiplication
    // - matmulParAssignShuffleHelper() helps to find the correct partition each item should be shuffled to for block matrix multiplication
    ...
    // We're going to shuffle a list of rows or columns <partition_number, <index_number, int that shows whether this is A, B, or C, value>>
    ...
    public static int getCol(long index,
                             int nRows, int nCols) {
        return ((int) index) % nCols;
    }

    // Given the partition/block number, the total number of partitions, as well as the matrix dimension
    // return the block row and column boundaries
    public static Tuple2<Tuple2<Integer, Integer>, Tuple2<Integer, Integer>>
    getPartitionRowsCols(long partNumber,
                         int b1, int b2,
                         int nRows, int nCols) {
    ...
    public static int getPartNumber(long index,
                                    int b1, int b2,
                                    int nRows, int nCols) {
        ...
        for (long partNum = 0; partNum < totalPart; partNum++) {
            Tuple2<Tuple2<Integer, Integer>, Tuple2<Integer, Integer>> t = getPartitionRowsCols(partNum, b1, b2, nRows, nCols);
            int cLowerRows = t._1()._1();
            int cHigherRows = t._1()._2();
            int cLowerCols = t._2()._1();
            int cHigherCols = t._2()._2();

            //System.out.println("partNum: " + partNum + ", cLowerRows: " + cLowerRows + ", cHigherRows: " + cHigherRows + ", cLowerCols: " + cLowerCols + ", cHigherCols: " + cHigherCols);
    ...
    // These indexer classes are used to translate indexes when we compose the constraints and assignments together
    // This is useful when the assignment indexes are spread out as a result of parallelization
    // getIndex(long index): given the virtual input index, what is the actual index in the assignment RDD
    ...
    public static class BlockIndexerC extends Indexer {
        public BlockIndexerC(long offset_,
                             int n1_, int n2_, int n3_, int b1_, int b2_, int b3_) {
            n1 = n1_;
            n2 = n2_;
            n3 = n3_;
            b1 = b1_;
            ...
            for (int i = 0; i < b1 * b3; i++) {
                Tuple2<Tuple2<Integer, Integer>, Tuple2<Integer, Integer>> t = getPartitionRowsCols(i, b1, b3, n1, n3);
                blockOffsets.add(t);
            }
        }
        ...
        public long getVirtualIndex(long index) {
            ...
            for (int i = 0; i < b1 * b3; i++) {
                ...
                //System.out.println("[BlockIndexerC::getVirtualIndex] curOffset is " + curOffset + ", cLowerRows: " + cLowerRows + ", cHigherRows: " + cHigherRows + ", cLowerCols: " + cLowerCols + ", cHigherCols: " + cHigherCols);
    ...
    public static class TransposeIndexer extends Indexer {
        ...
        public long getIndex(long index) {
            ...
            //System.out.println("[TransposeIndexer::getIndex] offset: " + offset + ", virtual index: " + index + ", real index: " + (offset + transposeIndex));
    ...
    public static <FieldT extends AbstractFieldElementExpanded<FieldT>>
    Iterator<Tuple2<Long, FieldT>>
    matmulParAssignHelper(final FieldT fieldFactory,
                          Long blockNumber,
        ...
        // this means we want rows in [cLowerRows, cHigherRows) from A, and columns in [cLowerCols, cHigherCols) from B
        ...
        for (Tuple2<Long, FieldT> x : AList) {
            ...
            //System.out.println("[AList] index is " + index + ", newIndex is " + newIndex + ", value is " + value);
        ...
        for (Tuple2<Long, FieldT> x : BList) {
            ...
            //System.out.println("[BList] index is " + index + ", r is " + r + ", c is " + c + ", newIndex is " + newIndex  + ", value is " + value);
            inputAssignment.set(aSize + newIndex, value);
        }

        //System.out.println("cSize is " + cSize + ", zSize is " + zSize + ", sSize is " + sSize + ", offset is " + offset);
        ...
        for (int c = 0; c < cSize; c++) {
            ...
            for (int i = 0; i < n2; i++) {
                ...
                //System.out.println("[C] c " + c + ", row is " + row + ", col is " + col + ", aIndex is " + aIndex + ", bIndex is " + bIndex + ", zIndex is " + assignmentOffsetIndexer.getIndex(zIndex) + ", z value is " + (inputAssignment.get(aIndex).mul(inputAssignment.get(bIndex))) + ", a: " + inputAssignment.get(aIndex) + ", b: " + inputAssignment.get(bIndex));
                assignment.set(zIndex,
                        new Tuple2<>(assignmentOffsetIndexer.getIndex(zIndex),
                                //offset + zIndex,
                ...
                if (i > 0) {
                    if (i == 1) {
                        ...
                        //System.out.println("[C] c " + c + ", z1Index is " + z1Index + ", z2Index is " + z2Index);
                        assignment.set(sIndex,
                                new Tuple2<>(assignmentOffsetIndexer.getIndex(sIndex),
                                        //offset + sIndex,
                        ...
                        //System.out.println("[C] c " + c + ", zIndex is " + zIndex + ", s1Index is " + s1Index + ", s2Index is " + s2Index);
                        assignment.set(s2Index,
                                new Tuple2<>(assignmentOffsetIndexer.getIndex(s2Index),
                                        //offset + s2Index,
            ...
            assignment.set(c,
                    new Tuple2<>(assignmentOffsetIndexer.getIndex(c),
                            //offset + c,
                            result));
        }
    ...
    // The format of the output should be (shuffling index, (Block row/col number, matrix type [A/B], [list of values in the matrix]))
    public static <FieldT extends AbstractFieldElementExpanded<FieldT>>
    Iterator<Tuple2<Long, Tuple2<Long, FieldT>>>
    matmulParAssignShuffleHelper(Tuple2<Long, FieldT> rec,
                                 int matType,
                                 final int n1, final int n2, final int n3,
        ...
        ArrayList<Tuple2<Long, Tuple2<Long, FieldT>>> data = new ArrayList<Tuple2<Long, Tuple2<Long, FieldT>>>();
    ...
    // This function is very similar to the non-block-based version, except now we take into account that the
    // C, Z, and S variables are numbered based on blocks
    public static <FieldT extends AbstractFieldElementExpanded<FieldT>>
    void matmulParConstraintGenHelper(ArrayList<Tuple2<Long, LinearTerm<FieldT>>> constraintArray,
                                      final FieldT fieldFactory,
                                      long index,
                                      final long n1, final long n2, final long n3,
                                      final long c1, final long c2,
                                      ConstraintType t,
                                      long aOffset,
                                      long bOffset,
        ...
        if (i < n2) {
            // // this is a product gate constraint
            // System.out.println("[Prod gate - constraintA] cIndex: " + cIndex + ", row: " + row + ", col: " + col + ", constraint i is " + constraintOffsetIndexer.getIndex(index));
            // System.out.println("[Prod gate - constraintA] A index: " + aOffsetIndexer.getIndex(aOffset + row * n2 + i));
            // System.out.println("[Prod gate - constraintB] B index: " + bOffsetIndexer.getIndex(bOffset + i * n3 + col));
            // System.out.println("[Prod gate - constraintC] Z index: " + (zOffset + (row * c2 + col) * (n2) + i));

            switch (t) {
                case constraintA:
                    constraintArray.add(new Tuple2<>(constraintOffsetIndexer.getIndex(index),
                            new LinearTerm<>(aOffsetIndexer.getIndex(aOffset + row * n2 + i),
                                    one)));
                    break;
                case constraintB:
                    constraintArray.add(new Tuple2<>(constraintOffsetIndexer.getIndex(index),
                            new LinearTerm<>(bOffsetIndexer.getIndex(bOffset + i * n3 + col),
                                    one)));
                    break;
                case constraintC:
                    constraintArray.add(new Tuple2<>(constraintOffsetIndexer.getIndex(index),
                            new LinearTerm<>(zOffset + (row * c2 + col) * n2 + i,
                                    one)));
                    break;
                default:
            ...
            if (i == (n2 - 1) || (i == 0 && n2 == 2)) {
                ...
                // System.out.println("[Sum gate - constraintA] cIndex: " + cIndex + ", row: " + row + ", col: " + col + ", constraint i is " + (index + constraintOffset));
                // System.out.println("[Sum gate - constraintA] Z1 index: " + (zOffset + (row * c2 + col) * n2));
                // System.out.println("[Sum gate - constraintB] Z2 index: " + (zOffset + (row * c2 + col) * n2 + 1));
                // System.out.println("[Sum gate - constraintC] S index: " + (sOffset + (row * c2 + col) * (n2 - 1) + i - 1));
                ...
                // System.out.println("[Sum gate - constraintA] cIndex: " + cIndex + ", row: " + row + ", col: " + col + ", constraint i is " + (index + constraintOffset));
                // System.out.println("[Sum gate - constraintA] Z1 index: " + (zOffset + (row * c2 + col) * n2 + i + 1));
                // System.out.println("[Sum gate - constraintB] S1 index: " + (sOffset + (row * c2 + col) * (n2 - 1) + i - 1));
                // System.out.println("[Sum gate - constraintC] S2 index: " + (sOffset + (row * c2 + col) * (n2 - 1) + i ));
    ...
    public static <FieldT extends AbstractFieldElementExpanded<FieldT>>
    Iterator<Tuple2<Long, LinearTerm<FieldT>>>
    matmulParConstraintGen(final FieldT fieldFactory,
        ...
        Tuple2<Tuple2<Integer, Integer>, Tuple2<Integer, Integer>> t = getPartitionRowsCols(blockNum, b1, b3, n1, n3);
        int cLowerRows = t._1()._1();
        int cHigherRows = t._1()._2();
        int cLowerCols = t._2()._1();
        int cHigherCols = t._2()._2();
        ...
        LinearIndexer newAssignmentOffsetC = new LinearIndexer(assignmentOffsetC.getIndex(0) + blockNum * (numConstraints + c1 * c2));
        ...
        //System.out.println("[matmulParConstraintGen] constraintOffset is " + constraintOffset.getIndex(0));
        ...
        for (long i = 0; i < numConstraints; i++) {
            matmulParConstraintGenHelper(constraintList, fieldFactory, i, n1, n2, n3, c1, c2, cType,
                    aOffset, bOffset,
                    assignmentOffsetA, assignmentOffsetB,
                    newAssignmentOffsetC, constraintOffset);
        }
    ...
    // Finally, in order to construct the constraints and the assignments, it takes in a constraint offset and an assignment offset (so we can later union the RDDs)
    public static <FieldT extends AbstractFieldElementExpanded<FieldT>>
    Tuple3<R1CSConstraintsRDD<FieldT>, JavaPairRDD<Long, FieldT>, Long>
    matmulParConstruct(final Configuration config, final FieldT fieldFactory,
                       JavaPairRDD<Long, FieldT> A, JavaPairRDD<Long, FieldT> B,
                       final int b1, final int b2, final int b3,
                       final int n1, final int n2, final int n3,
                       Indexer assignmentOffsetA, Indexer assignmentOffsetB, Indexer assignmentOffsetOutput,
        ...
        JavaPairRDD<Long, Tuple2<Long, FieldT>> aFullRDD = A.<Long, Tuple2<Long, FieldT>>flatMapToPair(
                data -> R1CSConstruction.<FieldT>matmulParAssignShuffleHelper(data, 0, n1, n2, n3, b1, b2, b3));

        JavaPairRDD<Long, Tuple2<Long, FieldT>> bFullRDD = B.<Long, Tuple2<Long, FieldT>>flatMapToPair(
                data -> R1CSConstruction.<FieldT>matmulParAssignShuffleHelper(data, 1, n1, n2, n3, b1, b2, b3));
        ...
        JavaPairRDD<Long, FieldT> oneFullAssignment = cogroupResult.flatMapToPair(x -> matmulParAssignHelper(fieldFactory, x._1(), x._2(),
                n1, n2, n3, b1, b2, b3,
        ...
        // config.endLog("[R1CSConstruction] witness calculation (oneFullAssignment count is " + assignmentCount + ")");
        ...
        JavaPairRDD<Long, LinearTerm<FieldT>> ALC = config.sparkContext()
                .parallelize(intList, numPartitions).flatMapToPair(blockNum ->
                        matmulParConstraintGen(fieldFactory, ConstraintType.constraintA,
                                assignmentOffsetA, assignmentOffsetB, assignmentOffsetOutput,
                                constraintOffset,
                                n1, n2, n3, b1, b2, b3, blockNum));

        JavaPairRDD<Long, LinearTerm<FieldT>> BLC = config.sparkContext()
                .parallelize(intList, numPartitions).flatMapToPair(blockNum ->
                        matmulParConstraintGen(fieldFactory, ConstraintType.constraintB,
                                assignmentOffsetA, assignmentOffsetB, assignmentOffsetOutput,
                                constraintOffset,
                                n1, n2, n3, b1, b2, b3, blockNum));
        ...
        JavaPairRDD<Long, LinearTerm<FieldT>> CLC = config.sparkContext()
                .parallelize(intList, numPartitions).flatMapToPair(blockNum ->
                        matmulParConstraintGen(fieldFactory, ConstraintType.constraintC,
                                assignmentOffsetA, assignmentOffsetB, assignmentOffsetOutput,
                                constraintOffset,
                                n1, n2, n3, b1, b2, b3, blockNum));
    ...
    matmulParConstructApp(final FieldT fieldFactory,
                          final int b1, final int b2, final int b3,
                          final int n1, final int n2, final int n3,
                          final Configuration config) {
        ...
        JavaPairRDD<Long, FieldT> aRDD = JavaPairRDD.fromJavaRDD(config.sparkContext().parallelize(A, numPartitions));
        JavaPairRDD<Long, FieldT> bRDD = JavaPairRDD.fromJavaRDD(config.sparkContext().parallelize(B, numPartitions));

        Tuple3<R1CSConstraintsRDD<FieldT>, JavaPairRDD<Long, FieldT>, Long> ret = matmulParConstruct(
                config, fieldFactory, aRDD, bRDD,
                b1, b2, b3, n1, n2, n3,
                assignmentOffsetA, assignmentOffsetB, assignmentOffsetC,
                constraintOffset);

        final R1CSConstraintsRDD<FieldT> constraints = ret._1();
        JavaPairRDD<Long, FieldT> oneFullAssignment = ret._2();

        JavaPairRDD<Long, FieldT> newARDD = aRDD.mapToPair(x -> {
            return new Tuple2<Long, FieldT>(x._1() + 1, x._2());
        });

        JavaPairRDD<Long, FieldT> newBRDD = bRDD.mapToPair(x -> {
    ...
    public static <FieldT extends AbstractFieldElementExpanded<FieldT>>
    R1CSConstraintsRDD<FieldT>
    equalityConstraintGen(final FieldT fieldFactory,
                          final Configuration config,
                          Indexer xOffset,
                          Indexer yOffset,
                          Indexer constraintOffset,
        ...
        // parallelize
        JavaPairRDD<Long, LinearTerm<FieldT>> constraintA = config.sparkContext()
                .parallelize(partitions, numPartitions).flatMapToPair(part -> {
                    final ArrayList<Tuple2<Long, LinearTerm<FieldT>>> A = new ArrayList<>();
                    long partSize = (part < numVariables % numPartitions) ? numVariables / numPartitions + 1 : numVariables / numPartitions;
                    long offset = 0;
                    if (numVariables % numPartitions == 0) {
                        ...
                        offset = (numVariables / numPartitions + 1) * (numVariables % numPartitions) + (numVariables / numPartitions) * (part - numVariables % numPartitions);
                    }
                    for (long i = offset; i < offset + partSize; i++) {
                        A.add(new Tuple2<>(constraintOffset.getIndex(i),
                                new LinearTerm<>(xOffset.getIndex(i), one)));
                    }
                    return A.iterator();
                });

        JavaPairRDD<Long, LinearTerm<FieldT>> constraintB = config.sparkContext()
                .parallelize(partitions, numPartitions).flatMapToPair(part -> {
                    final ArrayList<Tuple2<Long, LinearTerm<FieldT>>> A = new ArrayList<>();
                    long partSize = (part < numVariables % numPartitions) ? numVariables / numPartitions + 1 : numVariables / numPartitions;
                    long offset = 0;
                    ...
                    for (long i = offset; i < offset + partSize; i++) {
                        A.add(new Tuple2<>(constraintOffset.getIndex(i),
                                new LinearTerm<>((long) 0, one)));
                    }
                    return A.iterator();
                });

        JavaPairRDD<Long, LinearTerm<FieldT>> constraintC = config.sparkContext()
                .parallelize(partitions, numPartitions).flatMapToPair(part -> {
                    final ArrayList<Tuple2<Long, LinearTerm<FieldT>>> A = new ArrayList<>();
                    long partSize = (part < numVariables % numPartitions) ? numVariables / numPartitions + 1 : numVariables / numPartitions;
                    long offset = 0;
        ...
        final R1CSConstraintsRDD<FieldT> constraints = new R1CSConstraintsRDD<FieldT>(constraintA, constraintB, constraintC, numConstraints);
    ...
    public static <FieldT extends AbstractFieldElementExpanded<FieldT>>
    Tuple3<R1CSRelationRDD<FieldT>, Assignment<FieldT>, JavaPairRDD<Long, FieldT>>
    linearRegressionApp(final FieldT fieldFactory,
                        final Configuration config,
                        final int n, final int d, final int bn, final int bd) {
        ...
        JavaPairRDD<Long, FieldT> w = JavaPairRDD.fromJavaRDD(config.sparkContext().parallelize(wList, numPartitions));
        JavaPairRDD<Long, FieldT> y = JavaPairRDD.fromJavaRDD(config.sparkContext().parallelize(yList, numPartitions));

        JavaPairRDD<Long, FieldT> oneFullAssignment = config.sparkContext().parallelizePairs(Collections.singletonList(new Tuple2<>((long) 0, one)));
        ...
        JavaPairRDD<Long, FieldT> XT = X.mapToPair(x -> {
        ...
        Tuple3<R1CSConstraintsRDD<FieldT>, JavaPairRDD<Long, FieldT>, Long> retX2 = matmulParConstruct(config, fieldFactory,
                XT, X,
                bd, bn, bd, d, n, d,
                xtIndexer, xIndexer, outputOffsetX2, constraintOffsetX2);
        ...
        //System.out.println("rhsConstraintOffset is " + rhsConstraintOffset + ", rhsAssignmentsOffset is " + rhsAssignmentsOffset);
        ...
        JavaPairRDD<Long, FieldT> X2 = X2Assignments.flatMapToPair(x -> {
        ...
        Tuple3<R1CSConstraintsRDD<FieldT>, JavaPairRDD<Long, FieldT>, Long> retRHS = matmulParConstruct(config, fieldFactory,
                X2, w,
                bd, bd, 1, d, d, 1,
                RHSBlockIndexer, wIndexer, outputOffsetRHS, constraintOffsetRHS);
        ...
        Tuple3<R1CSConstraintsRDD<FieldT>, JavaPairRDD<Long, FieldT>, Long> retLHS = matmulParConstruct(config, fieldFactory,
                XT, y,
                bd, bn, 1, d, n, 1,
        ...
        R1CSConstraintsRDD<FieldT> finalConstraints = equalityConstraintGen(fieldFactory, config,
                RHSResultIndexer, LHSResultIndexer,
                finalConstraintOffset, d);
        constraints.union(finalConstraints);

        // We need to construct the final assignments
        JavaPairRDD<Long, FieldT> newXRDD = X.mapToPair(x -> {
            return new Tuple2<Long, FieldT>(x._1() + 1, x._2());
        });
        JavaPairRDD<Long, FieldT> newWRDD = w.mapToPair(x -> {
            return new Tuple2<Long, FieldT>(x._1() + 1 + n * d, x._2());
        });

        JavaPairRDD<Long, FieldT> newYRDD = y.mapToPair(x -> {
        ...
        final Assignment<FieldT> primary = new Assignment<>(Utils.convertFromPairs(oneFullAssignment.filter(e -> e._1 >= 0 && e._1 < numInputs).collect(),
                numInputs));
        ...
        final R1CSRelationRDD<FieldT> r1cs = new R1CSRelationRDD<>(constraints, numInputs, numVariables2 - numInputs);

        return new Tuple3<R1CSRelationRDD<FieldT>, Assignment<FieldT>, JavaPairRDD<Long, FieldT>>(r1cs, primary, oneFullAssignment);
    }
    ...
    public static <FieldT extends AbstractFieldElementExpanded<FieldT>>
    ArrayList<Tuple2<Long, LinearTerm<FieldT>>>
    linearCombinationConstraintsGen(final FieldT fieldFactory,
                                    ConstraintType cType,
        ...
        ArrayList<Tuple2<Long, LinearTerm<FieldT>>> output = new ArrayList<Tuple2<Long, LinearTerm<FieldT>>>();

        for (int i = 0; i < numInputs; i++) {
            for (int j = 0; j < length; j++) {
                switch (cType) {
                    case constraintA:
                        output.add(new Tuple2<Long, LinearTerm<FieldT>>(constraintOffset.getIndex(j),
                                new LinearTerm<FieldT>(inputOffset.get(i).getIndex(j), scalars.get(i))));
                        break;
                    case constraintB:
                        output.add(new Tuple2<Long, LinearTerm<FieldT>>(constraintOffset.getIndex(j),
                                new LinearTerm<FieldT>((long) 0, one)));
                        break;
                    case constraintC:
                        output.add(new Tuple2<Long, LinearTerm<FieldT>>(constraintOffset.getIndex(j),
                                new LinearTerm<FieldT>(outputOffset.getIndex(j), scalars.get(i))));
                        //System.out.println("[linearCombinationConstraintsGen - C] constraint " + constraintOffset.getIndex(j) + ", assign index " + outputOffset.getIndex(j) + ", scalar is " + scalars.get(i));
                        break;
                    default:
                        System.out.println("[R1CSConstruction.linearCombinationConstraintGen] unknown constraint");
                        break;
    ...
    public static <FieldT extends AbstractFieldElementExpanded<FieldT>>
    Iterator<Tuple2<Long, FieldT>>
    xMinusMeanAssignment(final FieldT fieldFactory,
                         Long rowNumber,
                         Tuple2<Iterable<Tuple2<Long, FieldT>>, Iterable<Tuple2<Long, FieldT>>> input,
        ...
        for (int i = 0; i < d; i++) {
            ...
            xMinusMean.add(new Tuple2<Long, FieldT>(outputIndexer.getIndex(rowNumber * d + i),
                    ret));

            //System.out.println("Idx is " + outputIndexer.getIndex(rowNumber * d + i) + ", X[" + i + "] is " + X.get(i)._2() +  ", mean[" + i + "] is " + mean.get(i)._2() + ", sub ret is " + ret);
        }
    ...
    public static <FieldT extends AbstractFieldElementExpanded<FieldT>>
    Iterator<Tuple2<Long, LinearTerm<FieldT>>>
    xMinusMeanConstraintsHelper(final FieldT fieldFactory,
                                ConstraintType cType,
                                Indexer inputOffset,
                                Indexer outputOffset,
                                Indexer constraintOffset,
        ...
        Tuple2<Tuple2<Integer, Integer>, Tuple2<Integer, Integer>> t = getPartitionRowsCols(blockNumber, bn, bd, n, d);
        ...
        for (int i = 0; i < numRows; i++) {
            ...
            ArrayList<Tuple2<Long, LinearTerm<FieldT>>> tmp = R1CSConstruction.<FieldT>linearCombinationConstraintsGen(fieldFactory, cType,
                    inputOffsets, scalars,
                    newOutputOffset,
                    newConstraintOffset,
                    1, cHigherCols - cLowerCols);
    ...
    public static <FieldT extends AbstractFieldElementExpanded<FieldT>>
    Tuple3<R1CSConstraintsRDD<FieldT>, JavaPairRDD<Long, FieldT>, Long>
    covVerify(final FieldT fieldFactory,
              final Configuration config,
              JavaPairRDD<Long, FieldT> X,
              JavaPairRDD<Long, FieldT> mean,
              JavaPairRDD<Long, FieldT> cov,
              final int n, final int d, final int bn, final int bd,
              Indexer xOffset, Indexer meanOffset, Indexer covIndexer,
        ...
        JavaPairRDD<Long, Tuple2<Long, FieldT>> meanTransformed = mean.flatMapToPair(x -> {
            ArrayList<Tuple2<Long, Tuple2<Long, FieldT>>> ret = new ArrayList<Tuple2<Long, Tuple2<Long, FieldT>>>();
        ...
        JavaPairRDD<Long, LinearTerm<FieldT>> ALCXm = config.sparkContext().parallelize(intList, numPartitions).flatMapToPair(blockNumber -> {
            return xMinusMeanConstraintsHelper(fieldFactory, ConstraintType.constraintA,
                    outputAssignmentIndexer, xOffset, constraintOffset,
                    n, d, bn, bd,
                    blockNumber);
        });

        // JavaPairRDD<Long, LinearTerm<FieldT>> BLCXm = config.sparkContext().parallelize(intList).flatMapToPair(blockNumber -> {
        ...
        JavaPairRDD<Long, LinearTerm<FieldT>> ALCMean = config.sparkContext().parallelize(intList, numPartitions).flatMapToPair(blockNumber -> {
            return xMinusMeanConstraintsHelper(fieldFactory, ConstraintType.constraintA,
                    newMeanOffset, xOffset, constraintOffset,
                    n, d, bn, bd,
                    blockNumber);
        });

        JavaPairRDD<Long, LinearTerm<FieldT>> BLC = config.sparkContext().parallelize(intList, numPartitions).flatMapToPair(blockNumber -> {
            return xMinusMeanConstraintsHelper(fieldFactory, ConstraintType.constraintB,
                    newMeanOffset, xOffset, constraintOffset,
                    n, d, bn, bd,
                    blockNumber);
        ...
        JavaPairRDD<Long, LinearTerm<FieldT>> CLC = config.sparkContext().parallelize(intList, numPartitions).flatMapToPair(blockNumber -> {
            return xMinusMeanConstraintsHelper(fieldFactory, ConstraintType.constraintC,
                    meanOffset, xOffset, constraintOffset,
                    n, d, bn, bd,
                    blockNumber);
        });


        R1CSConstraintsRDD<FieldT> constraints = new R1CSConstraintsRDD<FieldT>(ALCXm.union(ALCMean), BLC, CLC, n * d);
        ...
        JavaPairRDD<Long, FieldT> xmT = xm.mapToPair(x -> {
        ...
        JavaPairRDD<Long, FieldT> xmInput = xm.mapToPair(x -> {
            return new Tuple2<Long, FieldT>(outputAssignmentIndexer.getVirtualIndex(x._1()), x._2());
        });
        ...
        //System.out.println("xm2Constraint offset is " + xm2ConstraintOffset.getIndex(0) + ", xm2 assignment offset is " + xm2OutputAssignmentIndexer.getIndex(0) + ", xm assignment offset is " + xmIndexer.getIndex(0));
        Tuple3<R1CSConstraintsRDD<FieldT>, JavaPairRDD<Long, FieldT>, Long> xm2 = matmulParConstruct(config, fieldFactory,
                xmT, xmInput,
                bd, bn, bd, d, n, d,
                xmtIndexer, xmIndexer,
                xm2OutputAssignmentIndexer, xm2ConstraintOffset);
        ...
        LinearIndexer covNOutputAssignmentIndexer = new LinearIndexer(xm2OutputAssignmentIndexer.getIndex(0) + xm2NumAssignments);
        System.out.println();
        JavaPairRDD<Long, FieldT> covN = cov.mapToPair(x -> {
        ...
        BlockIndexerC rhsAssignmentIndexer = new BlockIndexerC(xm2OutputAssignmentIndexer.getIndex(0), d, n, d, bd, bn, bd);
        long finalConstraintOffsetNumber = xm2ConstraintOffset.getIndex(0) + xm2._1().size();
        LinearIndexer finalConstraintOffset = new LinearIndexer(finalConstraintOffsetNumber);
        R1CSConstraintsRDD<FieldT> finalConstraints = equalityConstraintGen(fieldFactory, config,
                rhsAssignmentIndexer, covNOutputAssignmentIndexer,
                finalConstraintOffset, d * d);
    ...
    public static <FieldT extends AbstractFieldElementExpanded<FieldT>>
    Iterator<Tuple2<Long, LinearTerm<FieldT>>>
    meanVerifyHelper(final FieldT fieldFactory,
                     ConstraintType cType,
                     Indexer xOffset,
                     Indexer meanOffset,
                     Indexer constraintOffset,
        ...
        LinearIndexer newConstraintOffset = new LinearIndexer(constraintOffset.getIndex(0) + cOffset);
        // if (cType == ConstraintType.constraintC) {
        //     System.out.println("cOffset: " + cOffset + ", newConstraintOffset's offset is " + newConstraintOffset.getIndex(0));
        // }
        ArrayList<Indexer> indexers = new ArrayList<Indexer>();
        ...
        ArrayList<Tuple2<Long, LinearTerm<FieldT>>> ret = R1CSConstruction.<FieldT>linearCombinationConstraintsGen(fieldFactory, cType,
                inputOffsets, scalars, newMeanOffset,
                newConstraintOffset,
                numInputs, cHigherCols - cLowerCols);
        return ret.iterator();
    }

    public static <FieldT extends AbstractFieldElementExpanded<FieldT>>
    R1CSConstraintsRDD<FieldT>
    meanVerify(final Configuration config,
               final FieldT fieldFactory,
               Indexer xOffset,
               Indexer meanOffset,
               Indexer constraintOffset,
        ...
        JavaPairRDD<Long, LinearTerm<FieldT>> ALC = config.sparkContext().parallelize(intList, numPartitions).flatMapToPair(blockNumber -> {
            return meanVerifyHelper(fieldFactory, ConstraintType.constraintA,
                    xOffset, meanOffset, constraintOffset,
                    n, d, bn, bd, one,
                    blockNumber);
        });


        JavaPairRDD<Long, LinearTerm<FieldT>> BLC = config.sparkContext().parallelize(intList2, numPartitions).flatMapToPair(blockNumber -> {
            return meanVerifyHelper(fieldFactory, ConstraintType.constraintB,
                    xOffset, meanOffset, constraintOffset,
                    1, d, 1, bd, one,
                    blockNumber);
        });

        JavaPairRDD<Long, LinearTerm<FieldT>> CLC = config.sparkContext().parallelize(intList2, numPartitions).flatMapToPair(blockNumber -> {
            return meanVerifyHelper(fieldFactory, ConstraintType.constraintC,
                    xOffset, meanOffset, constraintOffset,
                    1, d, 1, bd, NInput,
                    blockNumber);
        });
        ...
        //     System.out.println("clcList: " + v._1() + ": " + v._2().index() + " - " + v._2().value());
        // }
    ...
    public static <FieldT extends AbstractFieldElementExpanded<FieldT>>
    Tuple2<R1CSConstraintsRDD<FieldT>, JavaPairRDD<Long, FieldT>>
    gaussianFit(final Configuration config,
                final FieldT fieldFactory,
                JavaPairRDD<Long, FieldT> X,
                JavaPairRDD<Long, FieldT> mean,
                JavaPairRDD<Long, FieldT> cov,
                final int n, final int d,
                final int bn, final int bd,
                Indexer xOffset, Indexer meanOffset, Indexer covOffset,
                Indexer outputAssignmentOffset,
                Indexer constraintOffset) {

        // Get the mean constraints; we don't need extra witnesses since we only need X and the mean

        R1CSConstraintsRDD<FieldT> meanConstraints = meanVerify(config, fieldFactory,
                xOffset, meanOffset, constraintOffset,
                n, d, bn, bd);
        ...
        //     System.out.println("mean alcList: "  + v._1() + ": " +  v._2().index() + " - " + v._2().value());
        // }

        // Iterable<Tuple2<Long, LinearTerm<FieldT>>> meanBLC = meanConstraints.B().collect();
        // for (Tuple2<Long, LinearTerm<FieldT>> v : meanBLC) {
        //     System.out.println("mean blcList: " + v._1() + ": " + v._2().index() + " - " + v._2().value());
        // }

        // Iterable<Tuple2<Long, LinearTerm<FieldT>>> meanCLC = meanConstraints.C().collect();
        // for (Tuple2<Long, LinearTerm<FieldT>> v : meanCLC) {
        //     System.out.println("mean clcList: "  + v._1() + ": " +  v._2().index() + " - " + v._2().value());
        // }


        LinearIndexer covConstraintOffset = new LinearIndexer(constraintOffset.getIndex(meanConstraints.size()));
        Tuple3<R1CSConstraintsRDD<FieldT>, JavaPairRDD<Long, FieldT>, Long> covRet = covVerify(fieldFactory, config,
                X, mean, cov,
                n, d, bn, bd,
                xOffset, meanOffset, covOffset,
    ...
    public static <FieldT extends AbstractFieldElementExpanded<FieldT>>
    Tuple3<R1CSRelationRDD<FieldT>, Assignment<FieldT>, JavaPairRDD<Long, FieldT>>
    gaussianFitApp(final FieldT fieldFactory,
                   final Configuration config,
                   final int n, final int d, final int bn, final int bd) {
        ...
        for (long i = 0; i < XSize; i++) {
            XList.add(new Tuple2<>(i, tmp));
            int idx = (int) (i % d);
            meanList.set(idx, new Tuple2<Long, FieldT>((long) idx, meanList.get(idx)._2().add(tmp.mul(multipleNMinusOne))));
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < d * d; j++) {
                ...
                // FieldT subResult1 = fieldFactory.construct((i * d + row) * n).sub(fieldFactory.construct(meanLong.get(row)));
                // FieldT subResult2 = fieldFactory.construct((i * d + col) * n).sub(fieldFactory.construct(meanLong.get(col)));
        ...
        for (int i = 0; i < XSize; i++) {
            XList.set(i, new Tuple2<Long, FieldT>((long) i, XList.get(i)._2().mul(multipleN).mul(multipleNMinusOne)));
        }
        ...
        JavaPairRDD<Long, FieldT> cov = JavaPairRDD.fromJavaRDD(config.sparkContext().parallelize(covList, numPartitions));
        ...
        Tuple2<R1CSConstraintsRDD<FieldT>, JavaPairRDD<Long, FieldT>> ret = gaussianFit(config, fieldFactory,
                X, mean, cov,
                n, d, bn, bd,
                xOffset, meanOffset,
                covOffset, outputOffset, constraintOffset);

        R1CSConstraintsRDD<FieldT> constraints = ret._1();
        JavaPairRDD<Long, FieldT> oneFullAssignment = ret._2();

        // remap X, mean, and covariance to have the correct indices
        JavaPairRDD<Long, FieldT> newX = X.mapToPair(x -> {
            return new Tuple2<Long, FieldT>(x._1() + 1, x._2());
        });
        JavaPairRDD<Long, FieldT> newMean = mean.mapToPair(x -> {
            return new Tuple2<Long, FieldT>(x._1() + 1 + n * d, x._2());
        });
        JavaPairRDD<Long, FieldT> newCov = cov.mapToPair(x -> {
        ...
        final Assignment<FieldT> primary = new Assignment<FieldT>(Utils.<FieldT>convertFromPairs(oneFullAssignment.filter(e -> e._1 >= 0 && e._1 < numInputs).collect(),
                numInputs));
        config.endLog("[gaussianFitApp] primary generation");

        final R1CSRelationRDD<FieldT> r1cs = new R1CSRelationRDD<FieldT>(constraints, numInputs, numVariables - numInputs);
