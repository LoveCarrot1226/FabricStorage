...
import com.example.DizkZKP.reductions.r1cs_to_qap.R1CStoQAP;
import com.example.DizkZKP.relations.objects.Assignment;
import com.example.DizkZKP.relations.qap.QAPRelation;
import com.example.DizkZKP.relations.qap.QAPWitness;
import com.example.DizkZKP.zk_proof_systems.zkSNARK.objects.Proof;import com.example.DizkZKP.zk_proof_systems.zkSNARK.objects.ProvingKey;import scala.Tuple2;
...
public class SerialProver {
    ...
    Proof<G1T, G2T> prove(
        ...
        final QAPWitness<FieldT> qapWitness = R1CStoQAP
                .R1CStoQAPWitness(provingKey.r1cs(), primary, auxiliary, fieldFactory, config);
        ...
        evaluationAt = evaluationAt.add(VariableBaseMSM
                .serialMSM(auxiliary.elements(), provingKey.queryA().subList(numInputs, numVariables)));
        ...
        final Tuple2<G1T, G2T> evaluationBtPrimary = VariableBaseMSM
                .doubleMSM(primary.elements(), provingKey.queryB().subList(0, numInputs));
        final Tuple2<G1T, G2T> evaluationBtWitness = VariableBaseMSM
                .doubleMSM(auxiliary.elements(), provingKey.queryB().subList(numInputs, numVariables));
        ...
        final G1T evaluationHtZt = VariableBaseMSM
                .serialMSM(qapWitness.coefficientsH(), provingKey.queryH());
        ...
        G1T evaluationABC = VariableBaseMSM.serialMSM(
                auxiliary.subList(0, numWitness),
                provingKey.deltaABCG1().subList(0, numWitness));
