...
package com.example.DizkZKP.algebra.curves.barreto_naehrig;

import com.example.DizkZKP.algebra.curves.AbstractG2;
import com.example.DizkZKP.algebra.curves.barreto_naehrig.BNFields.BNFq;
import com.example.DizkZKP.algebra.curves.barreto_naehrig.BNFields.BNFq2;
import com.example.DizkZKP.algebra.curves.barreto_naehrig.BNFields.BNFr;
...
public abstract class BNG2<
    ...
    public BNG2T add(final BNG2T that) {
        ...
        final BNFq2T Z2Z2 = that.Z.square();

        final BNFq2T U1 = this.X.mul(Z2Z2);
        final BNFq2T U2 = that.X.mul(Z1Z1);

        final BNFq2T Z1Cubed = this.Z.mul(Z1Z1);
        final BNFq2T Z2Cubed = that.Z.mul(Z2Z2);

        final BNFq2T S1 = this.Y.mul(Z2Cubed);        // S1 = Y1 * Z2 * Z2Z2
        final BNFq2T S2 = that.Y.mul(Z1Cubed);   // S2 = Y2 * Z1 * Z1Z1
        ...
        final BNFq2T H = U2.sub(U1);                                 // H = U2-U1
        final BNFq2T S2MinusS1 = S2.sub(S1);
        final BNFq2T I = H.add(H).square();                          // I = (2 * H)^2
        final BNFq2T J = H.mul(I);                                   // J = H * I
        final BNFq2T r = S2MinusS1.add(S2MinusS1);                   // r = 2 * (S2-S1)
        final BNFq2T V = U1.mul(I);                                  // V = U1 * I
        final BNFq2T X3 = r.square().sub(J).sub(V.add(V));           // X3 = r^2 - J - 2 * V
        final BNFq2T S1_J = S1.mul(J);
        final BNFq2T Y3 = r.mul(V.sub(X3)).sub(S1_J.add(S1_J));      // Y3 = r * (V-X3)-2 S1 J
        final BNFq2T Z3 = this.Z.add(that.Z).square().sub(Z1Z1).sub(Z2Z2)
                .mul(H);                                                 // Z3 = ((Z1+Z2)^2-Z1Z1-Z2Z2) * H

        return this.construct(X3, Y3, Z3);
    }

    public BNG2T sub(final BNG2T that) {
    ...
    public BNG2T twice() {
        ...
        final BNFq2T A = this.X.square();                 // A = X1^2
        final BNFq2T B = this.Y.square();                 // B = Y1^2
        final BNFq2T C = B.square();                        // C = B^2
        BNFq2T D = this.X.add(B).square().sub(A).sub(C);
        D = D.add(D);                                       // D = 2 * ((X1 + B)^2 - A - C)
        final BNFq2T E = A.add(A).add(A);                   // E = 3 * A
        final BNFq2T F = E.square();                        // F = E^2
        final BNFq2T X3 = F.sub(D.add(D));                  // X3 = F - 2 D
        BNFq2T eightC = C.add(C);
        eightC = eightC.add(eightC);
        eightC = eightC.add(eightC);
        final BNFq2T Y3 = E.mul(D.sub(X3)).sub(eightC);     // Y3 = E * (D - X3) - 8 * C
        final BNFq2T Y1Z1 = this.Y.mul(this.Z);
        final BNFq2T Z3 = Y1Z1.add(Y1Z1);                   // Z3 = 2 * Y1 * Z1
    ...
    public boolean equals(final BNG2T that) {
        ...
        final BNFq2T Z2Squared = that.Z.square();

        if (!this.X.mul(Z2Squared).equals(that.X.mul(Z1Squared))) {
            return false;
        }

        final BNFq2T Z1Cubed = this.Z.mul(Z1Squared);
        final BNFq2T Z2Cubed = that.Z.mul(Z2Squared);

        return this.Y.mul(Z2Cubed).equals(that.Y.mul(Z1Cubed));
    }
}
